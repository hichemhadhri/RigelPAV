...
public class Main extends Application {
    ...
    private HBox drawControlBar(ObserverLocationBean obs, DateTimeBean db, TimeAnimator ta) throws IOException {
       ...
       latTextField.setFocusTraversable(false);
        ...
        position.setStyle("-fx-spacing: inherit;\n" + 
        ...
        ObservableList<String> list = FXCollections.observableArrayList(ZoneId.getAvailableZoneIds());
        list.sort(null);
        ...
        cb.setItems(FXCollections.observableList(Arrays.asList(NamedTimeAccelerator.values())));
        cb.setValue(NamedTimeAccelerator.TIMES_300);
        ta.acceleratorProperty().bind(Bindings.select(cb.valueProperty(),"accelerator"));
    ...
    private BorderPane drawInfos(ViewingParametersBean vp,SkyCanvasManager sm) {
        
        BorderPane infos = new BorderPane();
        infos.setStyle("-fx-padding: 4;-fx-background-color: white;");
        
        Text gauche = new Text(); 
        gauche.textProperty().bind(Bindings.format(Locale.ROOT, "Champ de vue : %.1f°", vp.fieldOfViewDeg()));
        
        Text center = new Text();
        center.textProperty().bind(sm.objectUnderMouseProperty().asString());
        
        Text droite = new Text(); 
        droite.textProperty().bind(Bindings.format(Locale.ROOT, "Azimut : %.2f°, hauteur : %.2f°",sm.mouseAzDegProperty(),sm.mouseAltDegProperty()));
        ...
        NumberStringConverter stringConverter =
              ...
              UnaryOperator<TextFormatter.Change> Filter = (change -> {
                  ...
                  stringConverter.fromString(newText).doubleValue();
                    boolean choice= bool ? GeographicCoordinates.isValidLonDeg(newDeg) : GeographicCoordinates.isValidLatDeg(newDeg);
                    return choice
                  ? change
                  : null;
                  } catch (Exception e) {
              ...
              TextFormatter<Number> TextFormatterr =
                new TextFormatter<>(stringConverter, 0.0, Filter);
